 ...
 */
public class NewHandFragment extends LazyLoadFragment {

    private static NewHandFragment fragment;
    public NewHandFragment(){

    }
    public static NewHandFragment getInstance() {
        if (fragment == null) {
            synchronized (NewHandFragment.class) {
                if (fragment == null) {
                    fragment = new NewHandFragment();
                }
            }
        }
        return fragment;
    ...
    protected int setLayoutId() {
        return R.layout.fragment_new_hand;
    }

    @Override
    protected void initViews() {
        xLoadingView=findViewById(R.id.xloading_view);
        refreshLayout=findViewById(R.id.srl_new_hand_fragment);
        refreshLayout.setColorScheme(R.color.word_red);
        recyclerView=findViewById(R.id.rl_new_hand_fragment);
    ...
    private void requestData() {
        ...
        params.put("ivstCategory","1");//5是全部，小微是2，综合是3，新手是1
