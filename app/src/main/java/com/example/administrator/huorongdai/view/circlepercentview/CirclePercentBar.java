...
public class CirclePercentBar extends View {
    ...
    public CirclePercentBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mArcColor = typedArray.getColor(R.styleable.CirclePercentBar_arcColor,ContextCompat.getColor(mContext, R.color.light_gray));
        ...
        mCenterTextColor = typedArray.getColor(R.styleable.CirclePercentBar_centerTextColor, ContextCompat.getColor(mContext, R.color.blue));
        arcStartColor = typedArray.getColor(R.styleable.CirclePercentBar_arcStartColor, ContextCompat.getColor(mContext, R.color.blue));
        typedArray.recycle();

        if (!isInEditMode()) {
            mArcWidth = ScreenAdapterTools.getInstance().loadCustomAttrValue(mArcWidth);
            mCenterTextSize = ScreenAdapterTools.getInstance().loadCustomAttrValue(mCenterTextSize);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawText(data, getWidth()/2-textBoundRect.width()/2, getHeight()/3+textBoundRect.height(),centerTextPaint);
        centerTextPaint.getTextBounds(text,0,text.length(),textBoundRect);
        canvas.drawText(text,getWidth()/2-textBoundRect.width()/2,  getHeight()/2+textBoundRect.height(),centerTextPaint);
    }
