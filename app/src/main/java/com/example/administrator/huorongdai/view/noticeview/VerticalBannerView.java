package com.example.administrator.huorongdai.view.noticeview;
...
public class VerticalBannerView extends LinearLayout implements BaseBannerAdapter.OnDataChangedListener {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mAnimDuration = array.getInteger(R.styleable.VerticalBannerView_animDurations,mAnimDuration);
        centerTextSize = array.getDimensionPixelSize(R.styleable.VerticalBannerView_bannerTextSize,centerTextSize);
        ...
        if (!isInEditMode()) {
            centerTextSize = ScreenAdapterTools.getInstance().loadCustomAttrValue(centerTextSize);
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(isInEditMode()){
            ...
            mDebugPaint.setTextSize(centerTextSize);
//            mDebugPaint.setTextSize(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,30,getResources().getDisplayMetrics()));//
            mDebugPaint.setStyle(Paint.Style.STROKE);
            canvas.drawText("123456",20,getHeight()*2/3,mDebugPaint);
        }
    }
